...
router.post(
  "/webhook",
  eventController.groupEvents,
  messageController.messageFilter
);
...
router.get("/display/:groupId", async (request, response) => {
  ...
  response.render("../view/displayMessages", {
    groupName,
    groupPicture,
    messageCollection,
    textByGem,
    ...
    file,
    text,
    logMessage1: "File JSON from get by ID: " + file,
...
router.get("/displayByDate/:groupId/:date", async (request, response) => {
  ...
  const groupName = await Getter.getGroupName(groupId);
  const groupPicture = await Getter.getGroupProfilePicture(groupId);
  const messageCollection = await fireStore.getTextsByDateForGemini(
    groupId,
    date
  );
  const textByGem = await gemini.flashText(messageCollection);
  ...
  response.render("../view/displayByDate", {
    groupName,
    groupPicture,
    file,
    text,
